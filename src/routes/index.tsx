    ...
    <div className="mx-auto max-w-[1400px] px-6 pt-10 pb-16">
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] gap-6">
        ...
        <section className="hairline border border-border bg-[color:var(--color-paper)] p-6">
          <header className="mb-4 flex items-baseline justify-between">
            <div>
              <p className="smallcaps text-[10px] text-muted-foreground">
                01 · Select trained source model
              </p>
              <h2 className="mt-0.5 font-serif text-lg text-foreground">
                Choose a trained city model
              </h2>
            </div>
            <Link
              to="/checkpoints"
              className="smallcaps text-[9px] text-muted-foreground hover:text-foreground"
            >
              Open library →
            </Link>
          </header>
          ...
          <div className="space-y-2">
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="font-serif text-sm text-foreground">
                      {c.training_city}, {c.training_country}
                    </span>
                    <span className="smallcaps text-[9px]" style={{ color: "#7a9461" }}>
                      ● Trained & available
                    </span>
                  </div>
                  ...
                  <dl className="mt-2 grid grid-cols-4 gap-2 font-mono text-[10px] text-muted-foreground">
                    <div>
                      <dt className="smallcaps text-[8px]">Acc</dt>
                      <dd className="text-foreground">{c.metrics.accuracy.toFixed(3)}</dd>
                    </div>
                    <div>
                      <dt className="smallcaps text-[8px]">F1</dt>
                      <dd className="text-foreground">{c.metrics.macroF1.toFixed(3)}</dd>
                    </div>
                    <div>
                      <dt className="smallcaps text-[8px]">Schema</dt>
                      <dd className="text-foreground">{c.feature_schema_version.split(" ")[0]}</dd>
                    </div>
                    <div>
                      <dt className="smallcaps text-[8px]">Trained</dt>
                      <dd className="text-foreground">{c.training_date.slice(0, 7)}</dd>
                    </div>
                  </dl>
                </button>
              );
            ...
            <UnavailableSlot
              label="Request local training"
              subtitle="When source ≠ target morphology"
            />
          </div>

          <p className="mt-4 text-[11px] italic text-muted-foreground">
            Only verified source checkpoints are shown. The architecture supports future checkpoints
            from any city worldwide.
          </p>
        </section>
        ...
        <section className="flex flex-col items-center justify-center px-2">
          <div className="flex flex-col items-center gap-2">
            ...
            <p
              className="smallcaps writing-mode-vertical text-[9px] text-muted-foreground"
        ...
        <section className="hairline border border-border bg-[color:var(--color-paper)] p-6">
          ...
          <div className="space-y-2">
            ...
            <Link
              to="/target"
              className="block rounded-sm border border-foreground bg-foreground px-4 py-3 text-left transition-colors hover:border-foreground/40"
            >
              <span className="font-serif text-sm text-background">+ Enter any city globally</span>
      ...
      <section className="mt-6 grid grid-cols-3 gap-6 border border-border bg-[color:var(--color-paper)] p-6">
        <div>
          ...
          <p className="mt-2 text-[11px] text-[color:var(--color-ink-deep)]">
            {source.interpretation_note}
          </p>
        </div>
        <div>
          <p className="smallcaps text-[10px] text-muted-foreground">Target data readiness</p>
          <p className="mt-1 font-serif text-base text-foreground">
            {target ? `${target.display_name}, ${target.country}` : "—"}
          </p>
          {target && (
            <>
              <p className="mt-1 font-mono text-[10px] text-muted-foreground">
                {audit.readiness.filter((r) => r.status === "pass").length}/{audit.readiness.length}{" "}
                checks pass
              </p>
              <p className="mt-2 text-[11px] text-[color:var(--color-ink-deep)]">
                {audit.readiness.filter((r) => r.status !== "pass").length} dataset gap(s) —
                affordability, quality, or coverage may be imputed.
              </p>
            </>
          )}
        </div>
      ...
      <p className="mt-8 text-[10px] text-muted-foreground">
        Source: thesis checkpoint registry · Audit logic deterministic per (source, target) pair ·
        Predictions on target cities are projections, never ground truth.
      </p>
    </div>
  );
}
